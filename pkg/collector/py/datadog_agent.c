...
PyObject* SetCheckMetadata(const char *checkID, const char *metadata);
...
static PyObject *set_check_metadata(PyObject *self, PyObject *args) {
    char *check_id, *metadata;

    PyGILState_STATE gstate;
    gstate = PyGILState_Ensure();

    // datadog_agent.set_check_metadata(check_id, metadata)
    if (!PyArg_ParseTuple(args, "ss", &check_id, &metadata)) {
      PyGILState_Release(gstate);
      return NULL;
    }

    PyGILState_Release(gstate);
    return SetCheckMetadata(check_id, metadata);
}
